...
@media (orientation:portrait){
  :root{--js-size:clamp(100px,36vw,200px);--knob-size:calc(var(--js-size)*0.25)}
  /* 竖屏允许垂直滚动，避免标题栏遮挡内容 */
  .container{overflow-y:auto;overflow-x:hidden;touch-action:pan-y;-webkit-overflow-scrolling:touch}
  /* 压缩标题栏高度 */
  .header{padding:3px 6px}
  .header h1{font-size:clamp(0.8rem,3vw,1rem);margin-bottom:2px}
  .status-bar{gap:3px;margin-top:2px;flex-wrap:wrap;justify-content:center}
  .status-item{font-size:clamp(0.5rem,2vw,0.62rem);padding:1px 3px;gap:2px}
  /* content 自适应剩余高度 */
  .content{
    display:grid;
  ...
  .buttons-container{grid-column:1/3;grid-row:1;padding:6px;gap:6px}
  .content>.joystick-container:first-child{grid-column:1;grid-row:2;min-width:0;overflow:hidden;padding:4px}
  .content>.joystick-container:last-child{grid-column:2;grid-row:2;min-width:0;overflow:hidden;padding:4px}
  .joystick-wrapper{margin-top:4px}
...
/*======== 开屏动画 ========*/
#splash{position:fixed;top:0;left:0;width:100%;height:100%;height:100dvh;z-index:9999;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#2a2a2a;transition:opacity .6s ease-out}
#splash.hide{opacity:0;pointer-events:none}
.splash-logo{font-size:clamp(2.4rem,10vw,5rem);font-weight:bold;color:#ffffff;letter-spacing:6px;animation:splashScale .8s ease-out}
@keyframes splashScale{0%{opacity:0;transform:scale(.4)}50%{opacity:1;transform:scale(1.08)}100%{transform:scale(1)}}
.splash-sub{font-size:clamp(.6rem,2.5vw,.85rem);color:rgba(255,255,255,.4);margin-top:clamp(6px,2vh,16px);letter-spacing:3px;animation:splashFadeIn 1s ease-out .4s both}
@keyframes splashFadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
.splash-dots{display:flex;gap:clamp(6px,2vw,12px);margin-top:clamp(14px,4vh,30px)}
.splash-dot{width:clamp(6px,1.5vw,10px);height:clamp(6px,1.5vw,10px);border-radius:50%;background:#00ff88;animation:splashDot 1.2s ease-in-out infinite}
.splash-dot:nth-child(2){animation-delay:.15s}
.splash-dot:nth-child(3){animation-delay:.3s}
.splash-dot:nth-child(4){animation-delay:.45s}
@keyframes splashDot{0%,100%{opacity:.2;transform:scale(.6)}50%{opacity:1;transform:scale(1)}}
</style>
</head>
<body>
<!-- 开屏动画 -->
<div id="splash">
  <div class="splash-logo">XTX</div>
  <div class="splash-sub">无人机遥控器</div>
  <div class="splash-dots"><div class="splash-dot"></div><div class="splash-dot"></div><div class="splash-dot"></div><div class="splash-dot"></div></div>
</div>
<div class="container">
  <!-- 顶部状态栏 -->
  <div class="header">
